...
class LanguageProcessor:
    ...
    def generate_matched_xlxs(self, language):
        #getting the pivot language, English 
        english_file_path = os.path.join(self.datasetPath, 'en-US.jsonl')
        eng = pd.read_json(english_file_path, lines=True)
        eng = eng[['id', 'utt', 'annot_utt']]

        if language == 'en-US.jsonl':
            return
        try:
            #get the passed language path and store in dataframe
            jsonl_file_path = os.path.join(self.datasetPath, language)
            df = pd.read_json(jsonl_file_path, lines=True)
            df = df[['id', 'utt', 'annot_utt']]
            #merge with pivot language 
            ...
            #convert merged dataframe to excel in the directory
    ...
    def train_translation_json(self, location):
        ...
        try:
            ...
            output_path = '../outputs/translated_train/'
